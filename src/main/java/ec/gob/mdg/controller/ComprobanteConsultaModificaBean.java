...
@Named
@ViewScoped
public class ComprobanteConsultaModificaBean implements Serializable {
	...
	// IR A DETALLE DE NOTAS DE CRDITO A CONSULTAR
	...
	public String irDetalleMod(String id) {
		...
		final Flash flash = context.getExternalContext().getFlash();
		flash.put("param", id);
		return "facturamodificadetalle?faces-redirect=true";
	}
	
	// IR A DETALLE DE NOTA DE CREDITO
		public String irDetalleNotasMod(String id) {
			final FacesContext context = FacesContext.getCurrentInstance();
			final Flash flash = context.getExternalContext().getFlash();
			flash.put("param", id);
			return "notascreditomodificaciondetalle?faces-redirect=true";
		}
